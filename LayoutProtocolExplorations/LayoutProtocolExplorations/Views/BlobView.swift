...
struct BlobView: View {
    ...
    var body: some View {
        TimelineView(.animation){ timeline in
            ...
            let angle = Angle.degrees(now.remainder(dividingBy: 3) * 120) // Transform the now value to loop every 360 degrees
...
//            Text("Value: \(x)")
            ...
            Canvas{ context , size in
                context.fill(path(in: CGRect(x: 0, y: 0, width: size.width, height: size.height), x: x, x2: x2), with:
                        .linearGradient(Gradient(colors: [Color(hex: "e85d04"),Color(hex: "ffd166")]), startPoint: CGPoint(x: 0, y: 0), endPoint: CGPoint(x: 300, y: 100))
                )
            }
    ...
    ///animate using the angles we're getting
    func path(in rect: CGRect, x: Double, x2: Double) -> Path {
        var path = Path()
        let width = rect.size.width
        let height = rect.size.height
        path.move(to: CGPoint(x: 0.9923*width, y: 0.42593*height))
        path.addCurve(to: CGPoint(x: 0.6355*width*x2, y: height), control1: CGPoint(x: 0.92554*width*x2, y: 0.77749*height), control2: CGPoint(x: 0.91864*width*x2, y: height))
        ...
        path.addCurve(to: CGPoint(x: 0.34086*width, y: 0.06324*height*x), control1: CGPoint(x: -0.0896*width, y: 0.43038*height), control2: CGPoint(x: 0.00248*width, y: 0.23012*height*x/2))
